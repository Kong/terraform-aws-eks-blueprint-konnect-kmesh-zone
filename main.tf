...
module "add_ons" {
  ...
  # Ideally, this should notlocal.enable_external_secretse` CRD
...
spec:
  ...
  - secretKey: token
    remoteRef:
      key: ${local.cp_token_aws_secret_name}
...
module "kong_mesh_helm" {
  ...
  description      = "Kong Konnect - Mesh Zone"
  namespace        = local.namespace
